...
ROW = {"기계번호": "P-101", "기계명": "염소 이송 펌프", "기계종류": "원심펌프",
       "처리량": "10 m3/h", "토출압력": "0.5 MPa", "회전수": "1750 rpm",
       "주요재질": "SUS316", "전동기용량": "7.5", "방호·보호장치 종류": "EOCR", "비고": "인버터"}
...
class Form14Tests(unittest.TestCase):
    def test_saved_rows_reach_the_statutory_form_and_pass_readiness(self):
        ...
        self.assertEqual(row[:2], ["P-101", "염소 이송 펌프"])
        self.assertIn("처리량 10 m3/h", row[2])
        self.assertIn("토출압력 0.5 MPa", row[2])
        self.assertIn("회전수 1750 rpm", row[2])
        self.assertEqual(row[3:5], ["SUS316", "7.5"])
