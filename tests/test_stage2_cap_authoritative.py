...
from engine.stage2.cap_report_authoritative import (
    cap_form6_msds_candidates,
    cap_form7_reference_candidates,
)
from engine.stage2.kosha_msds import reference_field_key
...
class Stage2CAPAuthoritativeSourceTests(unittest.TestCase):
        ...
        project.set_field(
            ...
            "KOSHA MSDS 참고자료",
            {
                ...
                "sections": {
                    "2": {
                        "items": [
                            ["유해성·위험성 분류", "급성 독성(흡입) : 구분 2"],
                            ["유해성·위험성 분류", "금속부식성 : 구분 1"],
                        ]
                    },
                    ...
                    "8": {
                        "items": [
                            ["노출기준", "TWA : 10 ppm"],
                            ["위험노출수준", "AEGL-2 60 ppm; ERPG-2 50 ppm"],
                        ]
                    },
                    "9": {
                        "items": [
    ...
    def test_kosha_candidates_fill_only_missing_review_fields(self):
        ...
        self.assertIn("물질상태", by_field)
        self.assertIn("폭발한계 하한(%)", by_field)
        self.assertIn("폭발한계 상한(%)", by_field)
        self.assertIn("독성구분-항목", by_field)
        self.assertIn("독성구분-구분", by_field)
        self.assertIn("위험노출수준", by_field)
        self.assertIn("허용농도값", by_field)
        self.assertIn("증기압(20℃, mmHg)", by_field)
        self.assertIn("부식성(유, 무)", by_field)
    ...
    def test_cap_report_starts_with_form1_and_keeps_candidate_source_boundaries(self):
