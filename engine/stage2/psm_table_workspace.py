...
PIPING = TableSpec(
    ...
    _cols(
        ("분류코드", "P&ID에서 배관 종류를 구분하는 코드(Class)입니다. 예: A1A"),
        ("유체의 명칭 또는 구분", "그 배관에 흐르는 물질이나 구분입니다. 예: 염소, 공정수"),
        ("설계온도", "배관의 설계온도(℃)입니다."),
        ("설계압력", "배관의 설계압력(MPa)입니다."),
        ("배관재질", "배관의 재질입니다. 예: SUS316L, CS"),
        ("개스킷 재질 및 형태", "개스킷의 재질과 형태입니다. 예: 스파이럴 와운드"),
...
_RELIEF_COLUMNS = (
    ("안전밸브·파열판 번호", "계기번호", "도면상의 안전밸브 또는 파열판 번호입니다. 예: PSV-101"),
    ...
    ("최종 배출·처리 지점", "배출 연결부위", "나간 물질이 최종 도착하는 곳입니다. 예: 대기, 스크러버, 플레어"),
    ("배출원인", "배출원인", "밸브가 작동하는 원인입니다. 예: 냉각수 차단, 화재, 밸브 오작동"),
    ("형식", "형식", "밸브 형식입니다. 예: 스프링식, 파일럿식, 파열판"),
)
RELIEF = TableSpec(
...
INTERLOCK = TableSpec(
    ...
    _cols(
        ("인터록번호", "다른 인터록과 구분되는 번호입니다. 예: IL-101"),
        ...
        ("가동중지범위", "멈추는 범위입니다. 예: 반응기 투입 중단, 전체 공정 정지"),
        ("점검주기", "인터록을 점검하는 주기입니다. 예: 연 1회"),
        ("비고", "참고할 내용을 적습니다.", "opt"),
    ),
    unique="인터록번호",
...
GAS_ALARM = TableSpec(
    ...
    _cols(
        ("감지기번호", "감지기의 고유 번호입니다. 예: GD-001"),
        ("감지대상", "감지하려는 물질입니다."),
        ("설치장소", "감지기를 설치한 장소(설비)입니다."),
        ("작동시간", "제조사 사양서의 응답 시간입니다."),
        ("측정방식", "센서의 측정 원리입니다. 예: 전기화학식, 적외선식"),
...
FIREPROOF = TableSpec(
    ...
    _cols(
        ...
        ("내화부위", "내화 처리한 부위입니다. 예: 기둥, 보, 지지 다리"),
        ("내화시험기준 및 시간", "내화 성능을 시험한 기준과 견디는 시간입니다. 예: KS F 2257, 2시간"),
        ("비고", "참고할 내용을 적습니다.", "opt"),
    ),
    conditional=True,
)
...
LOCAL_EXHAUST = TableSpec(
    ...
    _cols(
        ...
        ("배기 및 처리순서", "빨아낸 공기가 거쳐 가는 순서입니다. 예: 후드 → 덕트 → 스크러버 → 배기구"),
    ),
    conditional=True,
)
...
EXPERTS = TableSpec(
    ...
    _cols(
        ("책임분야", "그 전문가가 맡은 분야입니다. 예: 공정, 기계, 전기·계장, 안전"),
        ("성명", "전문가의 이름입니다."),
        ("소속회사", "전문가가 속한 회사입니다."),
        ("직책", "전문가의 직책입니다."),
        ("주요경력", "주요 경력입니다."),
