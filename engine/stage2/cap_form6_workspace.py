...
PROPERTY_COLUMNS: tuple[tuple[str, str, str], ...] = (
    ("물질상태", "물질 상태", "기체·액체·고체 중 하나입니다. 제품 SDS 제9항의 물리적 상태를 옮깁니다."),
    ("비중", "비중", "제품 SDS 제9항의 비중(상대밀도)입니다."),
    ("폭발한계 하한", "폭발한계 하한(%)", "제품 SDS 제9항의 폭발(인화) 하한입니다. 해당 없으면 '해당 없음'이라고 적습니다."),
    ("폭발한계 상한", "폭발한계 상한(%)", "제품 SDS 제9항의 폭발(인화) 상한입니다. 해당 없으면 '해당 없음'이라고 적습니다."),
    ("독성구분 항목", "독성구분 항목", "급성독성의 노출 경로입니다(예: 급성 독성(흡입))."),
    ("독성구분", "독성구분 구분", "급성독성 구분 번호입니다(예: 구분 2). SDS 제2항에서 확인합니다."),
    ("위험노출수준", "위험노출수준", "ERPG → AEGL → PAC → IDLH 순서로 값이 있는 첫 항목을 적습니다(매뉴얼 기준)."),
    ("허용농도값", "허용농도값", "SDS 제8항의 노출기준(TWA)입니다."),
    ("증기압", "증기압(20℃, mmHg)", "20℃에서의 증기압(mmHg)입니다. 다른 단위면 mmHg로 환산해 적습니다."),
    ("부식성", "부식성(유, 무)", "금속부식성이 있으면 '유', 없으면 '무'입니다."),
    ("SDS 파일명", "근거 SDS 파일명", "위 물성값을 옮겨 적은 제품 SDS 파일명입니다. KOSHA 값을 반영하면 자동으로 채워집니다."),
    ("SDS 개정일", "SDS 개정일", "그 SDS의 작성일 또는 개정일입니다."),
)
