...
OPTION_GROUPS: dict[str, tuple[str, ...]] = {
    ...
    "detector_measurement": ("전기화학식", "접촉연소식", "적외선식", "광이온화식(PID)", "반도체식", "열전도식", "기타(직접입력)"),
    "protected_class": ("갑종", "을종", "환경수용체"),
    ...
    "containment_type": ("방류벽", "방지턱", "트렌치", "집수조", "방유제", "기타(직접입력)"),
...
TABLE_DROPDOWNS: dict[str, dict[str, str]] = {
    ...
    "05_가스누출감지_경보장치": {
        "설치형태": "detector_type",
        "측정방식": "detector_measurement",
        "연동여부": "yes_no_na",
        "비상전원 여부": "yes_no_na",
    },
    "10_동력기계": {"기계종류": "machinery_type"},
...
TABLE_EXAMPLES: dict[str, tuple[tuple[Any, ...], ...]] = {
    "02_화학물질정보": (
        (
            "톨루엔", "108-88-3", "C7H8", 99.5, "액체", 15000, "kg", "2500 kg/day", "원료저장", "원료",
            "0.87", "1.2 vol%", "7.1 vol%", "TWA 50 ppm", "경구: SDS 제11항 확인 / 경피: SDS 제11항 확인 / 흡입: SDS 제11항 확인", "4 ℃", "480 ℃", "해당 없음",
            "급성독성(흡입)", "구분 4", "ERPG-2 300 ppm", "TWA 50 ppm", "28.4 mmHg (25℃)", "해당 없음",
            "toluene_company_SDS.pdf", "2026-06-01", "회사 제품 SDS 확인 예시"
        ),
        (
            "염소", "7782-50-5", "Cl2", 99.9, "기체", 2.5, "ton", "0.4 ton/day", "원료저장", "원료",
            "2.49 (공기=1)", "해당 없음", "해당 없음", "TWA 0.5 ppm", "경구: SDS 제11항 확인 / 경피: SDS 제11항 확인 / 흡입: SDS 제11항 확인", "해당 없음", "해당 없음", "해당 없음",
            "급성독성(흡입)", "구분 2", "ERPG-2 3 ppm", "TWA 0.5 ppm", "증기압 값 + 기준온도(SDS 제9항 확인)", "예: 반응 상대물질·조건은 SDS 제10항 확인",
            "chlorine_company_SDS.pdf", "2026-05-10", "독성가스 SDS 확인 예시"
        ),
        (
            "황산", "7664-93-9", "H2SO4", 98, "액체", 8, "m³", "1 m³/day", "폐수처리", "pH 조정제",
            "1.84", "해당 없음", "해당 없음", "TWA 0.2 mg/m³", "경구: SDS 제11항 확인 / 경피: SDS 제11항 확인 / 흡입: SDS 제11항 확인", "해당 없음", "해당 없음", "예",
            "피부부식성", "구분 1A", "자료 없음", "TWA 0.2 mg/m³", "자료 없음", "예",
            "sulfuric_acid_company_SDS.pdf", "2026-04-15", "부식성 액체 SDS 확인 예시"
        ),
        (
            "아세톤", "67-64-1", "C3H6O", 99, "액체", 2000, "L", "500 L/day", "세정", "세정제",
            "0.79", "2.5 vol%", "12.8 vol%", "TWA 500 ppm", "경구: SDS 제11항 확인 / 경피: SDS 제11항 확인 / 흡입: SDS 제11항 확인", "-20 ℃", "465 ℃", "해당 없음",
            "특정표적장기독성(1회)", "구분 3", "자료 없음", "TWA 500 ppm", "231 mmHg (25℃)", "해당 없음",
            "acetone_company_SDS.pdf", "2026-03-20", "세정·회수 공정 SDS 확인 예시"
        ),
    ),
    "03_설비정보": (
        ("TK-101", "톨루엔 저장탱크", "저장탱크", "원료저장", "톨루엔", 20, "m³",
         "", "2.5 m", "", "", "4.5 m", "", "", "20 m³",
         "0.49 MPa", "80 ℃", "0.15 MPa", "30 ℃", "SUS304", "SUS304", "PTFE", "1.0", "6 mm", "1 mm", "8 mm", "해당 없음", "10%", 15000, "PID-101", "산업안전보건법 적용 여부 확인", 50),
        ("R-201", "합성 반응기", "반응기", "반응", "원료 A/B", 5, "m³",
         "", "2.0 m", "2.5 m", "", "", "5 m³/batch", "", "",
         "1.0 MPa", "150 ℃", "0.45 MPa", "95 ℃", "SUS316L", "SUS316L", "Spiral Wound", "1.0", "8 mm", "1 mm", "10 mm", "해당 없음", "20%", 3200, "PID-201", "산업안전보건법 적용 여부 확인", 40),
        ("P-301", "제품 이송펌프", "펌프", "이송", "제품", 25, "m³/h",
         "", "", "", "", "", "", "", "",
         "0.8 MPa", "60 ℃", "0.4 MPa", "35 ℃", "SUS304", "SUS304", "PTFE", "해당 없음", "해당 없음", "해당 없음", "해당 없음", "해당 없음", "해당 없음", "", "PID-301", "해당 없음", 25),
        ("E-401", "공정 열교환기", "열교환기", "냉각", "공정유체", 120, "m²",
         "", "1.0 m", "", "", "3.0 m", "", "500,000 kcal/h", "",
         "1.2 MPa", "180 ℃", "0.5 MPa", "90 ℃", "SUS316L", "SUS316L", "Spiral Wound", "1.0", "7 mm", "1 mm", "9 mm", "해당 없음", "10%", "", "PID-401", "산업안전보건법 적용 여부 확인", 32),
        ("V-501", "중간제품 압력용기", "압력용기", "분리", "중간제품", 3, "m³",
         "", "", "", "", "", "", "", "",
         "1.5 MPa", "120 ℃", "0.7 MPa", "70 ℃", "SUS304", "SUS304", "Spiral Wound", "1.0", "8 mm", "1 mm", "10 mm", "해당 없음", "20%", 1800, "PID-501", "산업안전보건법 적용 여부 확인", 32),
    ),
    "04_안전밸브_파열판": (
        ("PSV-101", "TK-101", "안전밸브", "0.45 MPa", "1200 kg/h", "1300 kg/h", "25A", "40A", "톨루엔 증기", "증기", "0.15 MPa", "0.49 MPa", "WCB", "SUS316", "±3%", "스크러버", "화재", "PID-101", "저장탱크 보호 예시"),
        ("PSV-201", "R-201", "안전밸브", "0.90 MPa", "850 kg/h", "900 kg/h", "20A", "32A", "반응 혼합증기", "2상", "0.45 MPa", "1.0 MPa", "WCB", "SUS316", "±3%", "플레어 헤더", "반응폭주", "PID-201", "반응기 보호 예시"),
        ("RD-501", "V-501", "파열판", "1.30 MPa", "700 kg/h", "750 kg/h", "25A", "40A", "공정유체", "기체", "0.7 MPa", "1.5 MPa", "SUS316", "해당 없음", "제조사 사양", "비상벤트", "과압", "PID-501", "파열판 예시"),
    ),
    "05_가스누출감지_경보장치": (
        ("GD-101", "고정식", "TK-101 방유제 내", "톨루엔", "30초 이내", "접촉연소식", "10% LEL", "중앙제어실", "예", "HH 경보 시 원료 이송펌프 정지", "±3% F.S.", "월 1회 기능점검·연 1회 교정", "예", "GA-101", "인화성 증기 감지 예시"),
        ("GD-201", "고정식", "염소 저장실", "염소", "30초 이내", "전기화학식", "0.5 ppm", "중앙제어실·현장 경광등", "예", "HH 경보 시 염소 공급 차단", "제조사 사양 확인", "월 1회 기능점검·정기 교정", "예", "GA-201", "독성가스 감지 예시"),
        ("PD-301", "휴대식", "정비팀 비치", "복합가스", "제조사 사양", "전기화학식", "사업장 기준", "휴대기기", "아니오", "해당 없음", "제조사 사양 확인", "사용 전 기능점검", "해당 없음", "", "휴대용 감지기는 CAP 별지 제11호 자동기입 대상에서 제외"),
    ),
    "10_동력기계": (
        ("P-101", "원료 이송펌프", "원심펌프", "20 m3/h", "0.8 MPa", "1750 rpm", "", "", "", "", "7.5 kW", "SUS304", "커플링 가드·모터 과부하 보호", "톨루엔", "원료저장", "PID-101", "펌프 예시"),
        ("C-201", "공정 압축기", "왕복동압축기", "500 Nm3/h", "1.2 MPa", "900 rpm", "", "", "", "", "55 kW", "Carbon Steel", "압력·온도 인터록 및 가드", "공정가스", "압축", "PID-202", "압축기 예시"),
        ("AG-301", "혼합조 교반기", "교반기", "", "", "120 rpm", "0.4 m", "", "", "", "15 kW", "SUS316L", "회전부 가드·과부하 보호", "혼합액", "혼합", "PID-301", "교반기 예시"),
    ),
    "11_배관_개스킷": (
        ("PCL-150", "톨루엔", "SUS304", "50A", "0.49 MPa", "80 ℃", "PTFE", "10%", "해당 없음", "PID-101", "유기용제 배관 예시"),
        ("PCL-300", "황산", "PTFE Lined CS", "40A", "0.6 MPa", "60 ℃", "PTFE", "10%", "해당 없음", "PID-302", "부식성 유체 배관 예시"),
        ("PCL-600", "공정가스", "Carbon Steel", "80A", "1.2 MPa", "120 ℃", "Spiral Wound", "20%", "해당 없음", "PID-601", "가스 배관 예시"),
    ),
    "20_배출물질_처리시설": (
    ...
    "21_확산방지설비": (
        ...
        ("해당 없음", "", "", "", "", "", "", "", "", "", "", "사업장 전체 미적용인 경우 단독 행으로 입력"),
    ),
}
...
def _apply_table_dropdowns(wb, formulas: dict[str, str]) -> None:
    ...
    if "21_확산방지설비" in wb.sheetnames and "03_설비정보" in wb.sheetnames:
        ws = wb["21_확산방지설비"]
        headers = _header_map(ws)
        col = headers.get("대상 설비번호")
        if col:
            letter = ws.cell(4, col).column_letter
            _add_list_validation(
                ws,
                f"{letter}5:{letter}200",
                '=INDIRECT("\'03_설비정보\'!$A$5:$A$200")',
                prompt="03_설비정보에 등록한 설비번호(Tag No.)를 선택하세요. 사업장 전체 미적용이면 비워둘 수 있습니다.",
            )
...
def _annotate_identifiers(wb) -> None:
    notes = {
        ...
        ("21_확산방지설비", "대상 설비번호"): "03_설비정보의 설비번호(Tag No.)와 연결합니다. 확산방지설비의 필요용량·유효용량 검토대상을 특정하기 위한 값입니다.",
        ("21_확산방지설비", "필요용량 기준·근거"): "적용 시설기준, 내부 검토서 또는 회사가 확인한 기준을 구체적으로 적으세요. 프로그램은 일률적인 110% 등 임의 기준을 만들지 않습니다.",
        ("21_확산방지설비", "내부 차감용적(m3)"): "방류벽·방유제 내부 설비 등이 차지하는 용적을 적으세요. 차감이 없으면 빈칸이 아니라 0을 입력합니다.",
    }
