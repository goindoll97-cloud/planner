...
"""Mixture/component-aware CAP Stage 1 screening, scope, and holding.
...
legal threshold is held; and a multi-component reaction/simple-mixing facility
must identify which component CAS its process concentration belongs to.
"""
...
from .cap_engine import assess_cap as _base_assess_cap
from .cap_holding import CAPHoldingResult, assess_cap_holding as _base_assess_cap_holding
from .cap_holding_screen import FacilityStageScreen, screen_facility_stage as _base_screen_facility_stage
from .cap_quick_holding import compare_confirmed_declared_holding as _base_compare_confirmed_declared_holding
from .cap_scope_engine import assess_cap_scope as _base_assess_cap_scope
...
def screen_facility_stage(intake):
    """Mixture-aware CAP Appendix 4 facility-stage screen.

    Drop-in replacement for cap_holding_screen.screen_facility_stage: behaves
    identically for non-mixture intakes, and additionally screens each
    mixture's SDS Section-3 components (in parent-row/CAS-blanked order,
    following Appendix 3 -> Appendix 2 priority) when the intake has any.
    """
    mixtures = _mixture_rows(intake)
    if not mixtures:
        return _base_screen_facility_stage(intake)
    chemicals = intake.chemicals.copy()
    for parent in mixtures:
        if 0 < parent <= len(chemicals):
            chemicals.at[parent - 1, "CAS No."] = ""
    base = _base_screen_facility_stage(_clone_intake(intake, chemicals=chemicals))
    if not base.ready:
        return base
    ...
    for parent in sorted(mixtures):
        ...
        for _, component in _components_for_parent(intake, parent).iterrows():
            count += 1
            comp_hits, comp_blockers, claimed, ready = _screen_component_direct(_base_screen_facility_stage, intake, parent, component)
            if not ready:
                return FacilityStageScreen(legal_hits=hits, row_numbers=sorted(row_numbers), blockers=list(dict.fromkeys(blockers + comp_blockers)), messages=list(base.messages), ready=False)
    ...
    return FacilityStageScreen(legal_hits=unique_hits, row_numbers=sorted(row_numbers), blockers=list(dict.fromkeys(str(v) for v in blockers if str(v).strip())), messages=messages, ready=True)


def _component_scope(intake):
    ...
    for parent in sorted(_mixture_rows(intake)):
        ...
        for _, component in _components_for_parent(intake, parent).iterrows():
            ...
            if not values:
                continue
            result = _base_assess_cap_scope(_single_component_intake(intake, parent, component, values[-1]))
...
def assess_cap(intake):
    """Mixture-aware CAP scope/eligibility assessment.

    Drop-in replacement for cap_engine.assess_cap: behaves identically for
    non-mixture intakes, and additionally routes each mixture's SDS
    Section-3 components through Appendix 3/2 scope screening, falling back
    to the parent product's own SDS Section-2 (Appendix 1) only for
    components that claim nothing.
    """
    mixtures = _mixture_rows(intake)
    if not mixtures:
        return _base_assess_cap(intake)
    chemicals = intake.chemicals.copy()
    for parent in mixtures:
        if 0 < parent <= len(chemicals):
            chemicals.at[parent - 1, "CAS No."] = ""
            chemicals.at[parent - 1, "물질명(알면 입력)"] = ""
    base = _base_assess_cap(_clone_intake(intake, chemicals=chemicals))
    ...
    direct_scope, candidates = _component_scope(intake)
    ...
    direct_screen = screen_facility_stage(intake)
...
def assess_cap_holding(intake, facilities, legal_hits, required_row_numbers):
    """Mixture-aware CAP Appendix 4 maximum-holding assessment.

    Drop-in replacement for cap_holding.assess_cap_holding: behaves
    identically for non-mixture intakes, and for mixtures compares the
    parent's whole-mixture holding mass against each regulated component's
    own regulation-quantity thresholds (not the component's own mass).
    """
    mixtures = _mixture_rows(intake)
    ...
    if not any(row in mixtures for row in required):
        return _base_assess_cap_holding(intake, facilities, legal_hits, required)
    ...
    if nonmix_hits or nonmix_required:
        ...
        result = _base_assess_cap_holding(intake, nonmix_facilities, nonmix_hits, nonmix_required)
    ...
    for (parent, cas), hits in groups.items():
        ...
        result = _base_assess_cap_holding(_clone_intake(intake, chemicals=synthetic, facilities=comp_facilities), comp_facilities, hits, [parent])
        if result.status == "DB_NOT_READY":
            return result
        comparisons.extend(result.comparison_rows)
        facility_rows.extend(result.facility_rows)
    ...
    if blockers:
        return CAPHoldingResult(status="HOLD", label="화사계 최대보유량 판정보류", app4_ready=True, facility_rows=facility_rows, comparison_rows=comparisons, blockers=blockers, messages=list(dict.fromkeys(messages + ["혼합물은 규제성분별 적용 여부를 판단하되 최대보유량에는 혼합물 전체 질량을 사용합니다."])))
    ...
    return CAPHoldingResult(status=status, label=label, app4_ready=True, facility_rows=facility_rows, comparison_rows=comparisons, blockers=[], messages=list(dict.fromkeys(messages + ["혼합물 규제성분별로 동일한 혼합물 전체 최대보유량을 각각 규정수량과 비교했습니다."])))


def compare_confirmed_declared_holding(intake, legal_hits):
    """Mixture-aware reviewer-facing holding comparison.

    Drop-in replacement for cap_quick_holding.compare_confirmed_declared_holding:
    the quantity calculation itself is unaffected by mixtures (it already
    uses the parent's whole-mixture mass), but the reviewer-facing rows are
    relabeled with the actual regulated component's CAS/name instead of the
    parent product's.
    """
    result = _base_compare_confirmed_declared_holding(intake, legal_hits)
    mixtures = _mixture_rows(intake)
    if not mixtures:
        return result
