...
This module deliberately separates legal facts from UI/LLM behavior.  It only
returns a final '작성 필요/비대상' when quantity evidence and the legally
required exemption/major-facility facts are resolved.  Missing decisive facts
remain HOLD.

Current legal structure encoded here:
- 화학물질관리법 제23조제1항: 연구실, 학교, 대통령령/부령상 저영향 시설 등 면제
- 화학물질관리법 시행규칙 제19조제2항: 구체적 면제시설
- 화학사고예방관리계획서 작성 등에 관한 규정(2026-7) 제9조: 제19조제2항제10호 위임시설
- 같은 고시 제2조제1항제12의1·12의2: 1군/2군 정의
- 시행규칙 제19조제8항: 주요취급시설 정의

Important edge case: site-level maximum holding can exceed an upper quantity by
summing several facilities even though no *single* facility is a major handling
facility.  Therefore an upper-quantity site result alone does not prove Group 1;
when facility-level evidence is absent we ask/hold rather than force a group.
"""
...
EXEMPTIONS: tuple[CAPExemptionOption, ...] = (
    ...
    CAPExemptionOption(
        ...
        "운반 중인 차량 자체는 면제되지만 차량에 싣거나 내리는 작업은 이 면제에 포함되지 않습니다.",
        "「화학물질관리법 시행규칙」 제19조제2항제3호",
    ),
    CAPExemptionOption(
        "RULE_MILITARY",
        "군사기지·군사시설 내 유해화학물질 취급시설",
    ...
    CAPExemptionOption(
        ...
        "항공보안법상 보호구역 안에서 법정 항공운송사업자 또는 공항운영자가 설치·운영하는 시설입니다.",
        "「화학물질관리법 시행규칙」 제19조제2항제9호",
    ),
    CAPExemptionOption(
        "NOTICE_CONSUMER_PACKAGE",
        "소비자 판매용 포장제품 보관·진열시설",
        "유해화학물질이 포함된 제품을 포장된 상태로 소비자에게 판매하기 위해 보관·진열하는 시설입니다.",
        "「화학사고예방관리계획서 작성 등에 관한 규정」(2026-7) 제9조제1호",
    ),
    CAPExemptionOption(
        "NOTICE_WASTE_TEMP",
        "폐기물 처리를 위한 유해화학물질 폐기물 임시보관시설",
        "폐기물의 수집·운반·보관·재활용·처분을 위해 일시 보관하는 시설입니다.",
        "같은 고시 제9조제2호",
    ),
    CAPExemptionOption(
        "NOTICE_END_CONTROL",
        "공정 끝단의 대기·수질 오염물질 제거·감소 배출시설",
        "공정 마지막 단계에서 오염물질을 제거·감소하는 끝단 배출시설입니다. 중화·제거용 유해화학물질을 투입하기 위한 저장·사용시설은 제외되지 않습니다.",
        "같은 고시 제9조제3호",
    ),
    CAPExemptionOption(
        "NOTICE_EMBEDDED",
        "기계·장치에 내장되어 정상 사용 중 유출·누출·비산 우려가 없는 경우",
        "유해화학물질이 기계·장치 내부에 내장되어 정상적인 사용 과정에서 밖으로 나오지 않는 경우입니다.",
        "같은 고시 제9조제4호가목",
    ),
    CAPExemptionOption(
        "NOTICE_SOLID_PRODUCT",
    ...
    CAPExemptionOption(
        ...
        "CAS 7439-92-1 납을 고체 상태로 취급하면서 분말이나 미립자가 체류하거나 날릴 우려가 없는 경우입니다.",
        "같은 고시 제9조제4호라목",
    ),
    CAPExemptionOption(
        "NOTICE_NATURAL",
        "화학적으로 변형·추출·정제하지 않은 법정 자연상태 물질",
        "화평법 및 등록·신고 면제대상 규정에서 정한 자연상태 물질을 화학적으로 변형·추출·정제하지 않은 경우입니다.",
        "같은 고시 제9조제4호마목",
    ),
    CAPExemptionOption(
        "NOTICE_GAS_STATION",
...
def normalize_quantity_evidence(rows: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        band = str(row.get("quantity_band") or row.get("status") or "").upper()
        if "UPPER" in band or "상위 규정수량 이상" in str(row.get("quantity_band", "")):
            level = "UPPER"
        elif "LOWER" in band or "하위 이상" in str(row.get("quantity_band", "")):
            level = "LOWER"
        elif "BELOW" in band or "하위 규정수량 미만" in str(row.get("quantity_band", "")):
            level = "BELOW"
        else:
            continue
...
def assess_cap_final(
    ...
    *,
    ...
    """Return final CAP write/non-write/group decision, fail-closed.

    `exemption_answer`: NONE / EXEMPT / PARTIAL / UNKNOWN / UNANSWERED
    `major_facility_answer`: YES / NO / UNKNOWN / UNANSWERED. Asked only for an
    upper site-level result where facility-level evidence is not already known.
    """
    ...
    if answer == "PARTIAL":
        return CAPFinalDecision(
            "HOLD",
            "판정보류 — 일부 시설만 면제될 수 있습니다",
            blockers=["면제시설을 제외한 뒤에도 남는 취급시설의 물질별 최대보유량을 다시 산정해야 합니다."],
    ...
    if answer == "EXEMPT":
        ...
        return CAPFinalDecision(
            "NOT_REQUIRED",
            "작성 비대상 — 법정 면제시설",
            reasons=[option.label, option.plain_language],
            legal_basis=[option.legal_basis],
        )

    # NONE: no legal exemption applies to the relevant facilities.
    if upper:
        ...
        if major == "YES":
            return CAPFinalDecision(
                ...
                "작성 필요 — 1군 사업장",
                group="1군",
                reasons=["하나 이상의 물질이 사업장 최대보유량 기준 상위 규정수량 이상입니다.", "상위 규정수량 이상의 유해화학물질을 취급하는 주요취급시설을 운영합니다."],
                legal_basis=["「화학물질관리법 시행규칙」 제19조제8항", "「화학사고예방관리계획서 작성 등에 관한 규정」 제2조제1항제12의1, 제4조 및 제6조"],
            )
        if major == "NO":
            return CAPFinalDecision(
                "HOLD",
                "판정보류 — 상위기준이나 주요취급시설이 확인되지 않았습니다",
                blockers=["사업장 최대보유량은 여러 시설의 합일 수 있으므로, 상위 규정수량 이상 결과만으로 개별 주요취급시설 존재를 자동 확정할 수 없습니다."],
                next_question="상위 규정수량에 해당하는 물질의 시설별 취급량을 확인해 주요취급시설 여부를 검토해 주세요.",
