...
def _build_guide_sheet(wb: Workbook) -> None:
    ...
    ws["A1"] = "공정안전보고서·화학사고예방관리계획서 회사 입력파일 작성가이드"
    ...
    rows = [
        ...
        ["물질", "제품명·CAS·함량(%)", "기본 필수", "제품 SDS 제3항", "-", "추측하지 말고 모름", "공정안전보고서/화학사고예방관리계획서 물질기준", "포스겐 75-44-5, 100%"],
        ...
        ["법정 예외·특수조건", "「산업안전보건법 시행령」 제43조제2항 제외설비·「화학물질관리법」 제23조제1항 단서·고시 제9조 작성 면제 시설 등", "조건부 필수", "설비용도·인허가·운영자료", "해당없음", "모름", "제출·작성 의무 및 작성수준 결정", "05_최종판정조건·06_공정안전보고서_비고8제외수량 참고"],
    ]
...
def _build_documents_sheet(wb: Workbook) -> None:
    ...
    _style_header(ws, "A3:E3")
    rows = [
        ["공정안전보고서", "Y", "공정안전보고서 제출 대상 판정 후", "기존 공정안전자료 재활용", "2024년 작성본 보유 가정"],
...
def _build_psm_note8_sheet(wb: Workbook) -> None:
    ws = wb.create_sheet("06_공정안전보고서_비고8제외수량")
    ws.merge_cells("A1:G1")
    ws["A1"] = "6. 공정안전보고서 관련 「산업안전보건법 시행령」 별표 13 비고 제8호 제외수량 — 전문 가스 저장·판매시설에 해당할 때만 작성"
    ws["A1"].fill = PSM_FILL
    ws["A1"].font = Font(bold=True, size=13)
...
def build_legal_reference_workbook() -> bytes:
    ...
    psm_rows = [
        ...
        ["06_공정안전보고서_비고8제외수량", "별표13 호수 / 제조·취급 제외량 / 저장 제외량", "비고 제8호에 따라 제외할 수량을 항목별로 구분하고 비고 제7호 합산한 값(R)에 반영", "「산업안전보건법 시행령」 별표 13 비고 제7호·제8호", "별표 13 현행본", psm_url],
    ]
