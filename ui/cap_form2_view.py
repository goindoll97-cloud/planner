...
def _render_change_guide() -> None:
    ...
    with st.expander("이 대장은 언제 작성하고, 언제 제출하나요?", expanded=True):
        st.markdown("**작성 시점**: 계획서 적합통보 이후 사업장에 변경이 생기면 그때그때 변경일자·변경 전후 내용·후속조치를 기록해 관리합니다. 이 대장만 작성하거나 제출했다고 변경제출·영업 변경신고·변경허가가 끝나는 것은 아닙니다.")
        st.markdown("**제출 시점 ① 변경제출**: 작성규정 제11조제1항에 따른 변경제출 대상이면 **변경완료일 30일 이전까지** 화학물질안전원장에게 변경된 계획서를 제출하고, 적합 이후 관리한 이 대장을 포함합니다. 예를 들어 탱크 증설 완료 예정일이 2026년 11월 30일이라면 2026년 10월 31일까지 제출해야 합니다. ‘변경완료일’은 공사나 변경이 완료되는 날로 확인하세요. 영업 변경신고·변경허가의 기한과 혼동하지 마세요.")
        st.markdown("**예외: 주민소산계획 보완 통지를 받은 경우**에는 작성규정 제11조제3항에 따라 화학물질안전원장으로부터 계획서 제출을 **통지받은 날부터 60일 이내**에 변경된 계획서를 제출합니다. 통지 내용이 이 사유에 해당하는지 확인하세요.")
        st.markdown("**제출 시점 ② 매년 자체점검**: 시행규칙 제19조제8항의 **주요취급시설 운영자**는 적합통보를 받은 다음 연도부터 자체점검 결과와 이 대장을 매년 화학물질안전원장에게 제출합니다. 제출기한은 기준이 되는 적합통보일이 속한 분기의 마지막 날(3월 31일·6월 30일·9월 30일·12월 31일)입니다. 해당 사업장인지와 기준 적합통보일을 먼저 확인하세요.")
        st.markdown("**제출 시점 ③ 1군 재제출**: 1군 사업장의 계획서 재제출 주기는 작성규정 제12조에 따릅니다. 최초 적합통보 또는 해당 변경제출 적합통보일을 기준으로 5년이 되는 날 **이전**에 계획서를 재제출할 때, 적합 이후 관리한 이 대장을 포함합니다. 취급중단·휴업 등 법정 기간 제외나 개별 조건은 별도로 확인하세요. **5년은 모든 사업장이 대장만 내는 주기가 아닙니다.**")
        st.caption("예: 주요취급시설의 최초 적합통보가 2025년 5월이라면, 이듬해부터 매년 6월 30일까지 자체점검 결과와 대장을 제출하는 경우에 해당합니다. 탱크 증설이 별도로 계획서 변경제출 대상이면 연간 제출일까지 기다리지 말고 변경완료일 30일 이전 기한을 확인합니다. 날짜는 설명용 예시입니다.")
        st.markdown("근거: [「화학사고예방관리계획서 이행 등에 관한 규정」 제4조제2항](https://www.law.go.kr/LSW/admRulLsInfoP.do?admRulSeq=2100000279958) · [「화학사고예방관리계획서 작성 등에 관한 규정」 제11조·제12조](https://law.go.kr/LSW/admRulLsInfoP.do?admRulId=77224&efYd=0)")
