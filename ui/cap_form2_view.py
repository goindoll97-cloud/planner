...
def _render_change_guide() -> None:
    ...
    with st.expander("이 대장은 언제 작성하고, 언제 제출하나요?", expanded=True):
        ...
        st.markdown("**제출 시점 ① 변경제출**: 계획서 변경제출 대상이면 정해진 기한에 **변경된 계획서와 이 대장**을 함께 제출합니다. 변경제출 대상 여부와 기한은 작성규정 제11조에 따라 따로 확인합니다. 대장만 나중에 따로 제출하는 절차로 이해하지 마세요.")
        ...
        st.caption("예: 주요취급시설의 최초 적합통보가 2025년 5월이라면, 이듬해부터 매년 6월 30일까지 자체점검 결과와 대장을 제출하는 경우에 해당합니다. 탱크 증설이 별도로 계획서 변경제출 대상이면 6월까지 기다리지 말고 해당 변경제출 기한을 확인합니다. 날짜는 설명용 예시입니다.")
