...
def _render_change_guide() -> None:
    with st.expander("변경항목·변경종류·후속조치 작성 가이드"):
        ...
        st.markdown("**③ 변경의 종류는 현행 별지 제2호의 분류**입니다. 하나의 변경 건에서 여러 종류가 동시에 발생했다면 해당되는 항목을 모두 선택할 수 있습니다. 예를 들어 동일한 저장탱크의 위치를 변경하면서 재질도 변경했다면 '시설위치 변경'과 '시설 재질 변경'을 함께 선택합니다.")
